...
const nationalBoard = [
    ...
    },
    {
        ...
        niveau: '5e année',
        description:
            'Supervise la vie interne, les bureaux locaux et le développement des membres.',
    ...
    },
    {
        ...
        niveau: '6e année',
        description:
            'Représente l’association à l’international, maintient les partenariats et supervise la communication externe.',
    ...
    },
    {
        ...
        niveau: '5e année',
        description:
            'Gère le secrétariat général, l’archivage et les documents officiels de l’association.',
    ...
    },
    {
        ...
        niveau: '6e année',
        description:
            'Responsable de la gestion financière, des budgets et des rapports financiers.',
        ...
        members: [
            {
                ...
                niveau: '4e année',
                description:
                    'Supervise les activités du bureau local et coordonne les projets dans la ville de Lomé.',
            ...
            },
            {
                ...
                niveau: '3e année',
                description:
                    'Gestion administrative locale, suivi des activités et rédaction des comptes-rendus.',
        ...
        members: [
            {
                ...
                niveau: '5e année',
                description:
                    'Dirige les initiatives locales et coordonne les actions au sein de la région de Kara.',
...
<template>
    <PublicWrapper>
        ...
        <Section>
            <SectionTitle
                title="Bureau Exécutif National"
                subtitle="Membres actuels"
                align="center"
            />
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 mt-8">
                <div
                    v-for="member in nationalBoard"
                    :key="member.poste"
                    class="bg-muted rounded-xl p-4 shadow cursor-pointer hover:shadow-lg transition"
                ...
                >
                    ...
                    />
                    <div class="text-sm text-secondary font-bold">{{ member.poste }}</div>
        ...
        <Section class="bg-white dark:bg-zinc-950/40">
            ...
            <div v-for="board in localBoards" :key="board.location" class="mt-6">
                <h3 class="text-xl font-extrabold text-primary mb-4">{{ board.location }}</h3>
        ...
        <Section>
            <div class="flex justify-center items-center">
                <Button variant="transparent" @click="showOldBoards = !showOldBoards">
                    {{
                        showOldBoards
                            ? 'Cacher les anciens bureaux'
                            : 'Afficher les anciens bureaux'
                    }}
                </Button>
            </div>
            ...
            <div v-if="showOldBoards" class="mt-6 space-y-6">
                <div v-for="board in oldBoards" :key="board.year">
                    <h4 class="text-xl font-extrabold text-primary">{{ board.year }}</h4>
                    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 mt-3">
                        <div
                            v-for="member in board.members"
                            :key="member.poste"
                            class="bg-default rounded-xl p-4 shadow cursor-pointer hover:shadow-lg transition"
                            @click="openModal(member)"
                        >
                            <img
                                :src="member.photo"
        ...
        <Modal ref="modalRef" size="3xl" placement="top-center">
            <template #header>
                <span class="text-secondary">{{ activeMember?.poste }}</span> -
                {{ activeMember?.name }}
            </template>
            ...
            <div class="flex flex-col lg:flex-row gap-8">
                ...
                <div class="flex-1 space-y-4 text-default">
                    <p v-if="activeMember?.filiere">
                        <strong class="text-primary">Filière :</strong>
                        {{ activeMember.filiere }}
                    </p>
                    <p v-if="activeMember?.niveau">
                        <strong class="text-primary">Niveau :</strong> {{ activeMember.niveau }}
                    </p>
                    <p v-if="activeMember?.description">
                        <strong class="text-primary">Biographie :</strong>
                        {{ activeMember.description }}
                    </p>
                    <p v-if="activeMember?.mandat">
                        <strong class="text-primary">Programme de mandat :</strong>
                        {{ activeMember.mandat }}
                    </p>
                </div>
            </div>
