...
<template>
  <div class="w-full">
    <Link as="router-link" v-if="!hasChildren" :to="to" :variant="isActive ? 'primary' : 'neutral'" :class="[
      ...
      isActive
        ? 'bg-zinc-300 dark:bg-zinc-700/50 border-l-4 border-primary-500'
    ...
    <div v-else class="w-full">
      <button @click="open = !open" type="button"
        ...
        :class="isActive
            ? 'bg-zinc-300 text-primary-600 dark:text-primary-500 dark:bg-zinc-700'
            : 'text-zinc-700 dark:text-zinc-300 hover:bg-zinc-300/50 dark:hover:bg-zinc-700/50'
          ">
        <span>{{ label }}</span>
        <ChevronRightIcon :class="{ 'rotate-90': open }" class="w-4 h-4 transition-transform duration-300" />
      </button>
