...
const recentPosts = computed(() => allPosts
        ...
        <Section>
            <div class="grid grid-cols-1 lg:grid-cols-4 gap-8">
                <div class="lg:col-span-3 space-y-6">
                    <img :src="post?.image" alt="" class="w-full h-96 object-cover rounded-lg shadow" />
                    <div class="space-y-2">
                        ...
                        <div class="text-sm text-zinc-500 dark:text-zinc-400 flex gap-2 items-center">
                            <span>Par {{ post?.author }}</span> • <span>{{ post?.date }}</span>
                        </div>
                    </div>
                    <div class="prose prose-zinc dark:prose-invert text-zinc-700 dark:text-zinc-300"
                    ...
                    <div class="mt-10">
                        <h3 class="text-xl font-bold text-zinc-800 dark:text-zinc-200 mb-4">Commentaires</h3>
                        ...
                        <div class="space-y-4">
                            <div v-for="(comment, idx) in comments" :key="idx"
                                ...
                                <div class="flex-1">
                                    ...
                                    <p class="text-zinc-600 dark:text-zinc-500 text-sm">{{ comment.content }}</p>
                                </div>
                            </div>
                        </div>

                        <div class="mt-4">
                            <Textarea v-model="newComment" placeholder="Écrire un commentaire..." rows="4"
                ...
                <div class="space-y-6">
                    <div>
                        <h4 class="font-semibold text-zinc-800 dark:text-zinc-200 mb-2">Articles récents</h4>
                        <div class="space-y-2">
                            <div v-for="p in recentPosts" :key="p.id"
                                ...
                                <img :src="p.image" alt="" class="w-16 h-16 object-cover rounded-lg" />
                                <div class="flex-1">
                                    <p class="text-sm font-semibold text-zinc-800 dark:text-zinc-200">{{ p.title }}</p>
                                    <p class="text-xs text-zinc-500 dark:text-zinc-400">{{ p.date }}</p>
                                </div>
                            </div>
                        </div>
                    </div>
